...
const YouTubeChecker = require('../utils/youtubeChecker');
...
module.exports = {
    ...
    async execute(client) {
        ...
        const youtubeChecker = new YouTubeChecker(client);
        youtubeChecker.start();
        
        client.user.setActivity('YouTube Notifications', { type: 'WATCHING' });
...
async function cleanupOrphanedChannels(client) {
    try {
        ...
        for (const guild of guilds) {
            ...
            if (hasChanges) fs.writeFileSync(dataPath, JSON.stringify(guildData, null, 2));
        }
        ...
        if (fs.existsSync(statsDataPath)) {
            ...
            for (const [guildId, data] of Object.entries(statsData)) {
                ...
                if (!guild) continue;
                if (data.autoRestart) await updateServerStats(guild);
            }
        }
    } catch (error) {
...
async function updateServerStats(guild) {
    try {
        ...
        if (channelIds.all) updatePromises.push(updateChannelName(guild, channelIds.all, `👥 All Members: ${stats.total}`));
        if (channelIds.members) updatePromises.push(updateChannelName(guild, channelIds.members, `👤 Members: ${stats.humans}`));
        if (channelIds.bots) updatePromises.push(updateChannelName(guild, channelIds.bots, `🤖 Bots: ${stats.bots}`));
...
async function updateChannelName(guild, channelId, newName) {
    try {
        ...
        if (channel && channel.name !== newName) await channel.setName(newName);
    } catch (error) {
...
function calculateStats(guild) {
    ...
    return { total: totalMembers, humans: humanCount, bots: botCount };
}